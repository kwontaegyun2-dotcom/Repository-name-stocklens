...
US_UNIVERSE = [
    ...
    ("ARM.O", "암 홀딩스", "반도체"),
    # ⚠️ 6차 진단리포트(2026-09-23) 4-1 — 슈퍼마이크로컴퓨터(SMCI)는 AI GPU 서버를
    # "조립"하는 하드웨어 제조사고, 반도체를 직접 설계·제조하지 않는다(엔비디아 GPU를
    # 사다 서버에 넣는 쪽). 예전엔 "반도체"로 태깅해뒀는데, portfolio.py의 업종별
    # 비중과 themes.py의 "AI 반도체" 테마 노출이 둘 다 이 태그 하나에서 파생돼 항상
    # 똑같은 숫자로 나왔다(예: 삼성전자+SMCI+SK하이닉스=73.3% 가 "업종"에도 "테마"
    # 에도 동일하게 표시 — 두 지표가 사실상 같은 것을 두 번 보여줘 구분이 무의미했다).
    # 업종(표준 분류)에서는 분리하고, "AI 반도체" 테마(광의의 밸류체인 노출)에는
    # themes.py에서 별도로 다시 포함시킨다 — 업종과 테마가 서로 다른 숫자를 내야
    # 각자 의미가 있다.
    ("SMCI.O", "슈퍼마이크로컴퓨터", "AI서버·인프라"),
